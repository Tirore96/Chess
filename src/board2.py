...
def make_move(board,move,rights,player,king_pos,player_positions,chess_status,move_format="alg"):
    ...
    cur_player_positions = copy.copy(player_positions[-1])
    cur_chess_status = chess_status[-1]
    king_is_now_in_chess = is_king_now_in_chess(cur_board,cur_rights,cur_king_pos,player,cur_player_positions,chess_status)
...
#def try_move(cur_board,move,status,cur_king_pos):
#    x_1_alg,y_1_alg,x_2_alg,y_2_alg = move
#    index = board[x_1_alg,y_1_alg]
#    piece = pieces[index]
#    aux_move = []
#    if piece == "Pawn":
#        cur_board,aux_move = move_pawn(cur_board,move,status,player)
...
#    cur_king_pos = copy.copy(king_pos[-1])
#    cur_player_positions = copy.copy(player_positions[-1])
#    legal_move,status,new_rights = c_eval_pseudo_legal_move(cur_board,move,cur_rights,player,player_positions)
#    new_board,new_king_pos = try_move(cur_board,move,status,cur_king_pos)
#    king_is_now_in_chess = is_king_now_in_chess(new_board,new_rights,new_king_pos,player,cur_player_positions,chess_status)
#    retval = legal_move and not king_is_now_in_chess
#    return retval,status,new_rights,king_is_now_in_chess
